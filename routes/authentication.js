...
ROUTE.post('/login', findUser, async (req, res) => {
    const user_account = res.user
    try {
        if (!Boolean(user_account)) return res.status(404).send('user not found')
        const phraseVerified = await bcrypt.compare(req.body.password, user_account.password)
        if (phraseVerified !== true) return res.status(400).send("Please check your credentials")
        const accessToken = jwt.sign({
            userId: user_account._id,
            username: user_account.username,
            email: user_account.email
        }, process.env.ACCESS_TOKEN_SECRET)
        res.json({ accessToken, message: 'authenticaton successful!!' })
    } catch (error) {
